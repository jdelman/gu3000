...
void FrameBuffer::drawBitmap(int x, int y, const byte *bitmap, int width,
                             int height) {
  // Easy implementation
...
void FrameBuffer::drawChar(int x, int y, unsigned char c) {
  ...
  if (m_font_proportional) {
    drawBitmap(x, y, m_pfont_bitmap_ptr[i], m_pfont_width[i], m_font_height);
  } else {
    drawBitmap(x, y, &(m_font_bitmap[i * m_font_bytes]), m_font_width,
               m_font_height);
  }
}
...
void FrameBuffer::setFontProportional() {
  ...
  m_pfont_bitmap_ptr = (const byte **)realloc(m_pfont_bitmap_ptr,
                                              num_chars * sizeof(const byte *));
  m_pfont_width = (int *)realloc(m_pfont_width, num_chars * sizeof(int));
...
void FrameBuffer::writeFastVline(int x, int y, int vlength, int pen) {
  ...
  int i_start, i_end, i;
  static const byte bitmap1[8] = {0b11111111, 0b11111110, 0b11111100,
                                  0b11111000, 0b11110000, 0b11100000,
                                  0b11000000, 0b10000000};
  static const byte bitmap2[8] = {0b00000001, 0b00000011, 0b00000111,
...
const byte *FrameBuffer::bitmapContentTop(const byte *bitmap, int width,
                                          int height) {
  int height_bytes;
  byte bits;

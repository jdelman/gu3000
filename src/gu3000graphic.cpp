...
void GU3000Graphic::setDAD(word DAD) { m_dad = DAD; }

void GU3000Graphic::setBitmapOrder(int order) {
  GU3000GPIO::setBitmapOrder(order);
}

void GU3000Graphic::writeBitImage(word startAddress, word size,
                                  byte *imagedata) {
  int i;
  writeCommand(0x46);
...
void GU3000Graphic::writeAreaBitImage(word startAddress, word xbyte, word ybyte,
                                      byte *imagedata) {
  int i;
  writeCommand(0x42);
  writeWord(startAddress);
...
void GU3000Graphic::clearFrameBuffer() { FrameBuffer::clear(); }
...
void GU3000Graphic::show() {
  ...
  if (m_first_show) {
    ...
    for (i = 0; i < m_disp_areasize; i++) {
      if (m_drawing_addr + i >= m_disp_memsize)
        break;
      if (m_buf[m_drawing_addr + i] != buf[i])
        break;
    }
    if (i == m_disp_areasize)
      return; // m_buf == buf

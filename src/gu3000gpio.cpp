...
void GU3000GPIO::init() {
  init(VFD_RDY, VFD_WR, VFD_Data0, VFD_Data0 + 1, VFD_Data0 + 2, VFD_Data0 + 3,
       VFD_Data0 + 4, VFD_Data0 + 5, VFD_Data0 + 6, VFD_Data0 + 7);
}

void GU3000GPIO::init(int rdy, int wr, int d0, int d1, int d2, int d3, int d4,
...
void delayNanoseconds(unsigned int howLong) {
  ...
  do {
    clock_gettime(CLOCK_MONOTONIC, &tNow);
  } while ((tNow.tv_sec == tEnd.tv_sec) ? (tNow.tv_nsec < tEnd.tv_nsec)
...
void GU3000GPIO::setBitmapOrder(int order) { m_bitmapOrder = order; }
